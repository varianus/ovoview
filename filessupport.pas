...
function BuildFileList(const Path: string; MaskList: TStringList; const Attr: integer; const List: TStrings; Recurring: boolean): boolean;
    ...
    List.BeginUpdate;
      ...
      while Result do
        ...
        if Recurring and ((searchrec.Attr and faDirectory) <> 0) and (SearchRec.Name <> '.') and
          (SearchRec.Name <> '..') then
          BuildFileList(IncludeTrailingPathDelimiter(Directory + SearchRec.Name), MaskList,
            Attr, list, Recurring);
    ...
    while Result do
      begin
        if (SearchRec.Name <> '.') and
          ...
          if Recurring then
            BuildFolderList(Directory + SearchRec.Name, List, Recurring);
          end;
